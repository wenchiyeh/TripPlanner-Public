...
function TaiwanMap(props) {
  ...
  let url = `https://opendata.cwb.gov.tw/fileapi/v1/opendataapi/F-D0047-091?Authorization=CWB-CE3C77E2-B7C4-49F4-AE97-3E3F5E40BEAE&downloadType=WEB&format=JSON`
  let paths
  ...
  useEffect(() => {
    if (weatherData && weatherData.length > 0) {
      let resultData = weatherData.filter((item) => {
        return item.locationName === filter
      })
  ...
  useEffect(() => {
    if (result && result.length > 0) {
      ...
      if (highArr.length > 0 && lowArr.length > 0 && weatherArr.length > 0) {
        high = highArr[0].time[0].elementValue.value
